...
from sda.services.agent_output_formatter import AgentOutputFormatter
# MAX_TOOL_OUTPUT_LENGTH is now sourced from config via AgentOutputFormatter
# from sda.config import MAX_TOOL_OUTPUT_LENGTH # Ensure this is not accidentally re-imported if not needed
...
class AgentManager:
    ...
    def _get_tools_for_context(self, repo_id: int, branch: str) -> List[FunctionTool]:
        ...
        tools = [
            FunctionTool.from_defaults(fn=lambda q: AgentOutputFormatter.format_output(self.framework.analysis_engine.search_chunks_by_symbol(repo_id, branch, q)),
            ...
            FunctionTool.from_defaults(
                fn=lambda snippet: AgentOutputFormatter.format_output(self.framework.analysis_engine.find_similar_chunks_by_snippet(repo_id, branch, snippet)),
                name="find_similar_code_by_snippet", description="Finds code chunks semantically similar to a given code snippet."),
            FunctionTool.from_defaults(fn=lambda: AgentOutputFormatter.format_output(self.framework.find_dead_code_for_repo(repo_id, branch, 'agent')), name="find_dead_code",
                                       description="Starts a task to find potentially unused functions and classes. Returns a task object."),
            FunctionTool.from_defaults(fn=lambda: AgentOutputFormatter.format_output(self.framework.find_duplicate_code_for_repo(repo_id, branch, 'agent')),
                                       name="find_duplicate_code",
                                       description="Starts a task to find semantically similar code chunks. Returns a task object."),
            FunctionTool.from_defaults(fn=lambda: AgentOutputFormatter.format_output(self.framework.get_cpg_analysis(repo_id, branch)), name="get_code_graph_analysis",
                                       description="Gets a textual analysis of the Code Property Graph, including central nodes."),
            FunctionTool.from_defaults(fn=lambda: AgentOutputFormatter.format_output(self.framework.get_repository_stats(repo_id, branch)), name="get_repository_statistics",
            ...
            FunctionTool.from_defaults(fn=lambda symbol: AgentOutputFormatter.format_output(self.framework.navigation_tools.find_symbol_definition(symbol, repo_id, branch)),
                                       name="find_symbol_definition", description="Locates the definition of a symbol (e.g., a function or class name)."),
            FunctionTool.from_defaults(fn=lambda symbol: AgentOutputFormatter.format_output(self.framework.navigation_tools.find_symbol_references(symbol, repo_id, branch)),
                                       name="find_symbol_references", description="Finds all usages/references of a symbol."),
            FunctionTool.from_defaults(fn=lambda file_path: AgentOutputFormatter.format_output(self.framework.navigation_tools.get_file_outline(file_path, repo_id, branch)),
                                       name="get_file_outline", description="Provides a hierarchical view of the symbols within a single file."),
            FunctionTool.from_defaults(fn=lambda symbol: AgentOutputFormatter.format_output(self.framework.navigation_tools.get_call_hierarchy(symbol, repo_id, branch)),
                                       name="get_call_hierarchy",
                                       description="Traces incoming calls (callers) and outgoing calls (callees) for a specific function."),
            FunctionTool.from_defaults(
                fn=lambda file_path: AgentOutputFormatter.format_output(self.framework.navigation_tools.analyze_dependencies(file_path, repo_id, branch)),
                name="analyze_file_dependencies", description="Analyzes a file's incoming and outgoing dependencies based on code calls."),
            FunctionTool.from_defaults(
                fn=lambda path_prefix=None: AgentOutputFormatter.format_output(self.framework.list_files_in_repo(repo_id, branch, path_prefix)),
            ...
            FunctionTool.from_defaults(fn=lambda file_path: AgentOutputFormatter.format_output(self.framework.get_file_content_by_path(repo_id, branch, file_path)),
            ...
            FunctionTool.from_defaults(fn=lambda queries: AgentOutputFormatter.format_output(self.framework.search_files_for_text(repo_id, queries.split(','))),
            ...
            FunctionTool.from_defaults(fn=lambda: AgentOutputFormatter.format_output(self.framework.get_repository_status(repo_id)), name="get_git_status",
                                       description="Gets the git status (modified, new, untracked files) for the current repository."),
            FunctionTool.from_defaults(fn=lambda: AgentOutputFormatter.format_output(self.framework.get_repository_branches(repo_id)), name="list_repository_branches",
                                       description="Lists all available branches for the current repository."),
            # For add_repository, the raw output is likely fine, but we can format if needed.
            # For now, let's assume direct output is okay.
            ...
            FunctionTool.from_defaults(fn=lambda new_branch: AgentOutputFormatter.format_output(self.framework.analyze_branch(repo_id, new_branch, 'agent')),
                                       name="analyze_branch",
